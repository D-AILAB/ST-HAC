...
class CFG:
    ...
    CROP_MODE: str = "center"   # none | center | top_left | bottom_right | random
    CROP_SIZE: int = 20
    H: int = 20
    W: int = 20
...
def select_channels(arr: np.ndarray, channels: List[int]) -> np.ndarray:
    T, H, W, C_orig = arr.shape
    invalid = [ch for ch in channels if ch < 0 or ch >= C_orig]
    if invalid:
    ...
    if C_orig == 3:
        names_all = ["SMS", "Call", "Internet"]
    else:
        names_all = [f"Ch{i}" for i in range(C_orig)]
    sel_names = [names_all[i] if i < len(names_all) else f"Ch{i}" for i in channels]
...
def grid_coords(H: int, W: int, device):
    yy, xx = torch.meshgrid(torch.arange(H, device=device), torch.arange(W, device=device), indexing="ij")
    coords = torch.stack([yy, xx], dim=-1).reshape(-1, 2).float()
    return coords


def build_knn_idx_and_relpos(H: int, W: int, k: int, device):
...
class MultiPeriodicEmbed(nn.Module):
    def __init__(self, d_model: int, periods: List[int]):
        super().__init__()
...
class FrequencyAwareEncoder(nn.Module):
    def __init__(self, d_model: int, num_frequencies: int):
        super().__init__()
...
class EnhancedTemporalEncoder(nn.Module):
    def __init__(self, C_in: int, d_model: int, P: int, periods: List[int], num_freq: int):
        super().__init__()
        self.P = P
        self.d_model = d_model
        self.in_proj = nn.Linear(C_in, d_model)
        self.periodic_embed = MultiPeriodicEmbed(d_model // 2, periods)
        self.noise_embed = nn.Linear(C_in, d_model // 2)
        self.freq_encoder = FrequencyAwareEncoder(d_model, num_freq)
...
class OTProtoSpatial(nn.Module):
    def __init__(self, d_model: int, H: int, W: int, K: int, eps: float, tau: float, use_emb_dist: bool, dropout: float):
        super().__init__()
        self.K = K
    ...
    def forward(self, z, margin: torch.Tensor = None):
        B, N, D = z.shape
        device = z.device
        ...
        logits = -C / (self.eps + 1e-6)
        if margin is not None:
            logits = logits + margin

        T = torch.softmax(self.tau * logits, dim=-1)
...
class PARA(nn.Module):
    def __init__(self, d_model: int, H: int, W: int, tv_w: float, dropout: float):
        super().__init__()
        self.H, self.W = H, W
        self.tv_w = tv_w
    ...
    def forward(self, z, S):
        ...
        L = torch.cat(
            [torch.cat([La, pad], dim=-1),
             torch.cat([Lb, Lc], dim=-1)],
            dim=-2
        )
        ...
        SC = [
            dict(r=1.5, s0=0.25, sl=0.20, cb=0.70),
...
class SpatialEncoder(nn.Module):
    def __init__(self, d: int, H: int, W: int, n_proto: int):
        super().__init__()
...
class FinalTiGraphSpaFormer(nn.Module):
    def __init__(self, H: int, W: int, C: int, P: int, periods: List[int], num_freq: int):
        super().__init__()
        self.temporal = EnhancedTemporalEncoder(C, cfg.D_MODEL, P, periods=periods, num_freq=num_freq)
    ...
    def forward(self, x, cpd_hint=None):
        ...
        z_cell, _ = self.temporal(x, cpd_hint=cpd_hint)
...
def pinball_loss(y, yhat, tau: float):
...
def train_one_epoch(model, loader, min_v, max_v, optim):
    ...
    for X, y in loader:
        ...
        q_loss = (pinball_loss(yn, yp, 0.2) + pinball_loss(yn, yp, 0.5) + pinball_loss(yn, yp, 0.8)) / 3.0 \
            if cfg.USE_QLOSS else torch.tensor(0.0, device=yp.device)
...
def parse_args():
    ...
    p.add_argument("--train_npy", type=str, required=True)
    p.add_argument("--test_npy", type=str, required=True)

    p.add_argument("--channels", type=int, nargs="+", default=[0])
    p.add_argument("--crop_mode", type=str, default="center", choices=["none", "center", "top_left", "bottom_right", "random"])
    p.add_argument("--crop_size", type=int, default=20)

    p.add_argument("--p_in", type=int, default=25)
    ...
    p.add_argument("--periods", type=int, nargs="+", default=[48, 168])
    p.add_argument("--num_frequencies", type=int, default=16)

    p.add_argument("--save_dir", type=str, default="./checkpoints")
    p.add_argument("--no_save_preds", action="store_true")

    return p.parse_args()


def apply_args_to_cfg(args):
    cfg.TRAIN_NPY = args.train_npy
    cfg.TEST_NPY = args.test_npy

    cfg.CHANNELS = args.channels
    cfg.C = len(cfg.CHANNELS)

    cfg.CROP_MODE = args.crop_mode
    ...
    cfg.NUM_FREQUENCIES = args.num_frequencies

    cfg.SAVE_DIR = args.save_dir
    cfg.SAVE_PREDS = (not args.no_save_preds)
...
def main():
    ...
    print("TRAIN_NPY:", cfg.TRAIN_NPY)
    print("TEST_NPY :", cfg.TEST_NPY)
    print("Channels :", cfg.CHANNELS)
    print("Crop     :", cfg.CROP_MODE, cfg.CROP_SIZE)
    ...
    model = FinalTiGraphSpaFormer(
        H=cfg.H, W=cfg.W, C=cfg.C, P=cfg.P_IN,
        periods=cfg.PERIODS, num_freq=cfg.NUM_FREQUENCIES
    ).to(cfg.DEVICE)
    ...
    channel_str = "ch" + "_".join(map(str, cfg.CHANNELS))
    model_name = f"final_tigraph_spaformer_{crop_str}_{channel_str}.pt"
    ...
    for ep in range(1, cfg.EPOCHS + 1):
        ...
        print(f"[Ep {ep:03d}] train={tr_loss:.4f} | val={va_loss:.4f} | RMSEg={rmse_g:.3f} | R2g={r2_g:.3f} | "
    ...
    print("Global:", test_metrics["global"])
    print("Per-channel:", test_metrics["per_channel"])
